...
const props = defineProps<{ payload: QrPayload; busy?: boolean; existingAllergies?: string | null }>()
const emit = defineEmits<{ (e: 'confirm'): void; (e: 'cancel'): void }>()
const { t, locale } = useI18n()

const isReminder = computed(() => props.payload.k === 'r')
const isAllergy = computed(() => props.payload.k === 'a')
const kindLabel = computed(() => {
  if (props.payload.k === 'v') return t('ingest.vaccination')
  if (props.payload.k === 'r') return t('ingest.reminder')
  if (props.payload.k === 'a') return t('ingest.allergy')
  ...
  <section class="space-y-6 anim-rise">
    <div class="paper-card brackets p-6 md:p-8 relative overflow-hidden">
      ...
      <div aria-hidden class="absolute -right-6 -bottom-10 font-display text-[9rem] leading-none text-ink opacity-[0.05] select-none pointer-events-none">
        {{ payload.k === 'v' ? 'Rx' : payload.k === 'r' ? 'R' : payload.k === 'a' ? 'A' : 'Lab' }}
      </div>

      <div class="eyebrow mb-5">{{ $t('ingest.entry') }} · {{ kindLabel }}</div>

      <template v-if="isAllergy">
        <p class="font-display-wonk text-xl leading-snug whitespace-pre-line">{{ payload.n.trim() || $t('ingest.noKnownAllergies') }}</p>
        <div class="mt-6 pt-5 hairline-t text-sm text-muted-app italic font-display-wonk">
          {{ existingAllergies ? $t('ingest.allergyOverwriteNote') : $t('ingest.allergyFirstNote') }}
        </div>
      </template>
    ...
    <div class="flex gap-3">
      <button class="btn-primary flex-1" :disabled="busy" @click="emit('confirm')">
        {{ isAllergy ? $t('ingest.updateAllergies') : isReminder ? $t('ingest.scheduleReminder') : $t('ingest.addToLedger') }} <span aria-hidden>→</span>
      </button>
