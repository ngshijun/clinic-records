...
<template>
  <main class="min-h-dvh flex flex-col">
    <header class="max-w-[560px] w-full mx-auto px-6 pt-8 flex items-center justify-between">
      <div class="eyebrow"><span class="tick"></span>Scan</div>
      <router-link to="/home" class="folio underline underline-offset-4 decoration-[var(--color-rule)]">← back</router-link>
    </header>
    ...
    <section class="flex-1 flex flex-col items-center justify-center px-6 py-10 gap-8">
      <div class="text-center space-y-2 anim-rise">
        ...
        <h1 class="font-display text-4xl leading-tight">Hold still.</h1>
        <p class="text-sm text-muted-app max-w-[32ch]">Align the clinic's QR inside the frame.</p>
      </div>

      <!-- Viewfinder with corner brackets + scanning line -->
    ...
    <footer class="max-w-[560px] w-full mx-auto px-6 pb-6 flex items-center justify-between text-xs">
      ...
      <router-link to="/home" class="btn-ghost !py-2 !px-4">Cancel</router-link>
    </footer>
  </main>
</template>
